...
while True:
    ...
    for i in msg_chunk:
        generate_key(key)
        m = decrypt(msg_chunk[j])
        j = j+1
        enc_chunk.append(m)
    enc_msg = ''.join(map(str,enc_chunk))

    #m = m.decode()
    print(s_name, "decrypted:", enc_msg)
